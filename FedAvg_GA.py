...
# 交叉
def Cross(population, ids, crossNum, crossProbability):
    k1 = int(ids[0])
    k2 = int(ids[1])
...
# 变异
def Mutation(population, mutationRandom):
    startPos = np.random.randint(0, len(population[0]), 1, dtype=int)[0]

    for chromosome in population:
        ran = random.random()
        if ran < mutationRandom:
            if chromosome[startPos] == 0:
                chromosome[startPos] = 1
            else:
                chromosome[startPos] = 0
...
def rouletteWheel(fitnessAdd):
    ...
    while len(ids) != 2:
        ...
        for i in range(len(fitnessAdd)-1):
            if fitnessAdd[0] >= p:
                ids.append(0)
                break
            elif p > fitnessAdd[len(fitnessAdd)-1]:
                ids.append(len(fitnessAdd)-1)
                break
            elif fitnessAdd[i] < p <= fitnessAdd[i+1]:
                if p not in ids:
                    ids.append(i)
                break
...
if __name__ == '__main__':
    ...
    for i in range(federatedNum):
        ...
        while iterateNum < GAIterateNum:
            ...
            fitnessAdd = []
            for fit in fitness:
                fitnessAdd.append(fit)
                if len(fitnessAdd) > 1:
                    fitnessAdd[-1] += fitnessAdd[-2]

            newPopulation = population.copy()

            newNum = 0
            while newNum < chromosomeNum:
                ids = rouletteWheel(fitnessAdd)
                newPopulation[newNum], population[newNum+1] = Cross(population, ids, int(clientNum/2), 0.9)
                newNum += 2

            population = newPopulation.copy()

            # 变异
            Mutation(population, 0.05)
